...
#### Sync/Move Google documents in their native format ####

Native Google documents can be synced or moved within a Google My Drive or between a Google Shared Drive. Sometimes, you may want to keep these files in their native Google format rather than the default rclone setting which converts them to a format like docx,xlsx,pptx,svg. 

Specifically, Google Sheets has a number of powerful Google-only functions that are not compatible with Microsoft Excel or xlsx format, like QUERY and ARRAYFORMULA. QUERY runs Google's Visualisation API Query Language across data. Converting native files to xlsx will destroy these functions. 

To keep Google docs in their native format look at using the `--drive-server-side-across-configs` flag. (Eg. rclone copy remote1:"Location1" remote2:"Location2" --drive-server-side-across-configs). 

If you want to use a service account to impersonate a Google Drive user, then there are some complications using the `--drive-server-side-across-configs` flag. The service account is linked to a Google account which is different from the user that is being impersonated, and as such the impersonated user may not have permissions to sync or move directories or files that are not native Google docs. Where they don't, the rclone log will indicate a Google 404 error. Using the --drive-server-side-across-configs flag whilst impersonating a user will only sync or copy Google docs but no other files or directories. Removing the --drive-server-side-across-configs flag from the sync/copy will work on all files, but your Google docs will be converted to formats like docx, xlsx, pptx.
